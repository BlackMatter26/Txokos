...
app.get('/api/invite_list/:eventId', UserController.getInviteList, (req, res) => {
...
app.post('/api/add_invite/:eventId', HostController.getUserId, HostController.addInvite, (req, res) => {
  res.status(200).json(res.locals.userEventsId);
});

/* --------------- Attendee --------------- */
// Get all invited events for a specific attendee EVEN the RSVP ones
app.get('/api/attendee_invited_event/:attendeeId', AttendeeController.getInvitedEvents, (req, res) => {
  res.status(200).json(res.locals.attendeeInvitedEvents);
});

// Get all RSVP'd events for a specific attendee NOT only invited
app.get('/api/attendee_rsvp_event/:attendeeId', AttendeeController.getRSVPEvents, (req, res) => {
  res.status(200).json(res.locals.attendeeRSVPEvents);
});

// Setting RSVP to true, telling us they are going
app.post('/api/rsvp_event/:attendeeId/:eventId', AttendeeController.rsvpEvent, (req, res) => {
...
app.post('/api/claim_food/:attendeeId/:eventId/:eventFoodId', AttendeeController.getRSVPId, AttendeeController.claimFood, (req, res) => {
